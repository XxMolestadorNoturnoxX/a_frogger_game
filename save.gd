...
@export var desativo: String
@export var astivo: String


func _on_body_entered(_body: Node2D) -> void:
	Global.position = posicao
	if astivo:
		for spawner in get_tree().get_nodes_in_group(astivo):
			spawner.process_mode = Node.PROCESS_MODE_INHERIT
		if astivo not in Global.areas:
			Global.areas.append(astivo)
	if desativo:
		for spawner in get_tree().get_nodes_in_group(desativo):
			spawner.process_mode = Node.PROCESS_MODE_DISABLED
		Global.areas.erase(desativo)
